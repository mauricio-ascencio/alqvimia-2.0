...
function WorkflowsView() {
  ...
  const executeStep = async (step) => {
    ...
    // Si es un message_box, mostrar el diálogo de Windows con variables resueltas
    if (step.action === 'message_box' || step.action === 'pause') {
      ...
      // Esperar hasta que se cierre el mensaje
      await new Promise(resolve => {
        const checkClosed = setInterval(() => {
          // Verificar si el mensaje se cerró
          if (!showMessageBox) {
      ...
      if (varName) {
        setVariables(prev => {
          ...
          if (existing >= 0) {
            const updated = [...prev]
            updated[existing] = { ...updated[existing], value: varValue }
            return updated
          }
          return [...prev, { name: varName, type: 'string', value: varValue }]
        })
      }
      await new Promise(resolve => setTimeout(resolve, 100))
    } else if (step.action === 'log_message') {
      // Log con variables resueltas
      console.log('[Workflow Log]', resolvedParams?.message || step.label)
      await new Promise(resolve => setTimeout(resolve, 100))
    } else {
      // Simular ejecución de otros pasos
  ...
  const handleAIGenerate = async () => {
    ...
    // Simular generación de workflow con IA
    setTimeout(() => {
      const generatedSteps = [
        { id: `step_${Date.now()}_1`, action: 'browser_open', icon: 'fa-window-maximize', label: 'Abrir Navegador', params: {} },
        { id: `step_${Date.now()}_2`, action: 'navigate', icon: 'fa-compass', label: 'Navegar a URL', params: { url: 'https://example.com' } },
        { id: `step_${Date.now()}_3`, action: 'wait_element', icon: 'fa-hourglass-half', label: 'Esperar Elemento', params: {} },
        { id: `step_${Date.now()}_4`, action: 'click', icon: 'fa-mouse-pointer', label: 'Hacer Clic', params: {} },
        { id: `step_${Date.now()}_5`, action: 'message_box', icon: 'fa-window-restore', label: 'Message Box', params: { message: 'Proceso completado' } }
      ]
  ...
  return (
    <div className="view" id="workflows-view">
      <div className="workflow-studio" id="workflowStudio">
        ...
        <div className={`studio-left-panel ${leftPanelCollapsed ? 'collapsed' : ''}`}>
                  ...
                  <div key={category.id} className={`action-category ${expandedCategories[category.id] ? 'expanded' : ''}`}>
                    ...
                    <div className="category-items">
                      {category.actions.map(action => (
                        <div key={action.action} className={`action-item ${action.isContainer ? 'is-container' : ''}`} draggable="true" onDragStart={(e) => handleDragStart(e, action)}>
                          <i className={`fas ${action.icon}`}></i><span>{action.label}</span>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        <div className={`studio-right-panel ${rightPanelCollapsed ? 'collapsed' : ''}`}>
          <div className="panel-header">
