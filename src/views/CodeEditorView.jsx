...
function CodeEditorView() {
  ...
  const [dictationTarget, setDictationTarget] = useState('editor') // 'editor' o 'aiPrompt'
  const dictationTargetRef = useRef('editor') // Ref para evitar stale closure en speech recognition
    ...
    if (SpeechRecognition) {
      ...
      recognitionRef.current.onresult = (event) => {
        ...
        if (voiceMode === 'dictation') {
          ...
          // Cuando el resultado es final, insertar en el editor
          if (finalTranscript) {
            insertDictatedText(finalTranscript)
            setDictationBuffer('')
          }
        } else {
  ...
  // Usamos dictationTargetRef para evitar stale closures en el handler de speech recognition
    ...
    <div className="code-editor-container">
      ...
      <div className="editor-main">
        ...
        <div className="editor-panel">
          ...
          {/* Dictation Live Preview Banner */}
          {isListening && voiceMode === 'dictation' && (dictationBuffer || dictationTarget === 'aiPrompt') && (
            <div className={`dictation-live-banner ${dictationTarget === 'aiPrompt' ? 'ai-target' : ''}`}>
              <div className="dictation-live-icon">
                <i className="fas fa-microphone"></i>
              </div>
              <div className="dictation-live-text">
                <span className="dictation-live-label">
                  {dictationTarget === 'aiPrompt' ? 'Dictando al Prompt IA:' : 'Dictando al Editor:'}
                </span>
                <span className="dictation-live-content">
                  {dictationBuffer || (dictationTarget === 'aiPrompt' ? 'Esperando...' : '')}
                </span>
              </div>
              <div className="dictation-live-hint">
                {dictationTarget === 'aiPrompt'
                  ? 'Haz clic en el editor para dictar código'
        ...
        {showAIPanel && (
          <div className="ai-panel">
            ...
            <div className="ai-custom-prompt">
              ...
              <textarea
                ref={aiPromptRef}
                placeholder="Escribe tu instrucción para la IA... (o usa el dictado de voz)"
                value={aiPrompt}
              ...
              {isListening && voiceMode === 'dictation' && dictationTarget === 'aiPrompt' && (
                <div className="dictation-target-indicator">
                  <i className="fas fa-microphone"></i>
                  <span>Dictando aquí...</span>
                </div>
              )}
              <button
